...
(async () => {
  ...
  check('连接入口改图标但保留明确无障碍名称和提示', qa('.bcard__connect').every((b) => b.querySelector('svg') && b.getAttribute('aria-label') === '连接到另一张卡片' && b.title === '连接到另一张卡片'));
  check('场号标题同属紧凑标题行', !!q('.bcard__head .bcard__no + .bcard__title'));
  check('整场删除仍明确标注并保留图标', q('.bcard__del-scene').getAttribute('aria-label') === '删除整场' && !!q('.bcard__del-scene svg'));
  check('所有缩放入口保留且不显示文字', qa('.bcard__resize').length === 4 && qa('.bcard__resize').every((b) => !b.textContent.trim() && b.getAttribute('aria-label')));
