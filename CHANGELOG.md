...
## v1.3.0-alpha.18.10（编辑撤销与全文查找，本地候选）

- 修复段落中间回车后，聚焦段落仍显示旧尾部的问题；富文本与软换行边界使用一致光标偏移，不复制尾文。
- 剪切、粘贴和跨段选区替换改为单次撤销事务，与前后打字分开；撤销/重做立即同步当前编辑框。复制不修改正文或历史，字面尖括号/换行不丢失。
- 新增“查找正文”面板与 ⌘/Ctrl+F 入口、匹配数量、结果列表、上下处跳转、大小写选项和 Esc 关闭。查找只读，高亮不写回工程、不增加历史，不进入 PDF。
- 补充编辑与搜索回归、真实 Chromium 按键/组合输入验证及核心维护红线。保留现用应用，不改用户剧本；仅本地候选，未公开发布。

## v1.3.0-alpha.18.9（PDF 导出修复）

- 修复短对白的跟随分页约束串联后续人物、造成过早翻页和大片空白；保留人物与对白同页、长对白续页、双列及全文行连续性。不改用户字体、行距或正文换行。
- 图文 PDF 改用独立内存会话中的短地址加载，避免多 MB 内嵌图片造成 `ERR_INVALID_URL`；不将剧本写入临时 HTML，不改变图文原位布局。失败信息不再回显整份 HTML。
- 新增大负载加载/错误清理回归及真实 Electron 多 MB 图片、短对白 PDF 用例；不包含用户剧本。
- 独立打包测试版，不覆盖正在运行的应用。实际验收见 [RELEASE_ALPHA_18_9.md](RELEASE_ALPHA_18_9.md)，公开安装附件以 GitHub Releases 为准。

## v1.3.0-alpha.18.8（自由板卡片精简）

- 场号与场景标题同排，连接与删除改为带明确提示及无障碍名称的图标；操作区保留固定空间，长标题最多两行。
- 去掉缩放文字和灰色方块，保留原18px手柄与原缩放操作；图片卡不再重复显示“图片”标签，保留图片、关联与操作入口。
- 场景色改为日夜分别适配的整卡淡染色，编辑区透出底色，选中外框独立保留。
- 运行时改动仅限自由板组件与屏幕专用样式，不改正文、PDF、保存格式、卡片坐标/尺寸、连接及删除撤销逻辑。补充图标语义与紧凑布局回归。
- 安装包仍为 macOS Apple Silicon 测试版，ad-hoc签名而非Apple公证；发布与实机验证状态见本版交付记录。

## v1.3.0-alpha.18.7（本地输入性能优化，未发布）

- 正文输入不再每字复制/比较整个含图工程；仅更新当前段落与元素列表，图片、设置和未改段落保持引用。共用原有撤销/重做/连续输入合并规则，通用结构修改仍隔离历史快照。
- 自动保存改为直接订阅状态，不再驱动 App 整树随输入刷新；保持原来的约 900ms 停笔保存与最新文件关联，不推迟持久化。
- 增加不可变快照、历史等价、无效操作、自动保存调度/卸载回归和合成工程性能对比；两条 CI 纳入输入专项。增加独立 Electron 连续输入/中文组合输入/自动保存重载测试。
- 未修改正文键盘/Tab/选择事件、分页算法、PDF 导出、统计口径、幕归属或素材卡交互。尚不代表用户实际输入法、任意工程规模或所有平台均已验收。

## v1.3.0-alpha.18.6（本地候选，待推送和发布）

- 本版合并本轮故事板幕管理、统计分析、自由板与写作界面完善；未改现用应用及用户剧本。
- 自由板筛选时关系线仅连接两端均可见的卡片；选择、改色和删除限定当前可见卡片。删除整场及批量删除场景增加明确确认，可一次撤销；取消不改变选择或历史。
- 自由板增加可关闭的淡网格，随平移/缩放对齐，不吸附、不改旧坐标；无保存尺寸的场景卡默认更高，连接端点同步，已保存尺寸原样保留。
- 写作多选控件使用原位上下文工具区，避免挤压折字；导航副标题显示页码与幕信息，不再重复场景标题。
- 修复场景条被 flex 撑满的问题：未达到目标保留真实空白，超目标按当前篇幅归一显示，提示仍显示占目标比例；保留单一打字机、原有计算权重和精确场标题跳转。
- 进度栏页数按实际内容占宽，采用本机 Snell Roundhand 英式圆手体，统一标尺与目标/素材工具间距；不改变进度栏高度或正文排版。
- 新增 board-polish、writing-controls、progress-bands 专项并纳入 CI；统计样式、自由板样式与工具栏样式限定各自作用域，不改变分页/导出几何及正文键盘处理。

- 统计页重组为四项总览、幕/场景词字数占比、人物发言分布、地点/时段与待处理事项；长场景/人物表分页与筛选，点击可通过既有入口定位场次标题。
- 统一统计过滤和计数口径；阅读时长不再暗示成片时长，发言次数不再误称台词段数。实际涉及页数按当前排版去重，标题空页不计；未知地点不猜测。
- 保留原有精确名称改名及撤销动作，括号名称分别编辑、合并统计。CSV 同口径输出并防止表格公式注入。
- 此次运行时代码仅涉及 ReportsView、专用 reports 模型与限定样式，不修改其他界面、共用 stats/store、分页/导出/保存或用户剧本。

## v1.3.0-alpha.18.5（本地候选，待发布）

- 故事板幕分组：未归幕置顶，支持折叠、命名、场数与实际排版涉及页数；拖动高亮和前后插入线明确落点。
- 单卡/多卡“移至”菜单，复选框、Shift 连选与 ⌘/Ctrl 增减选；批量拖动保持相对顺序并一次撤销。
- 根据用户批准的新规则，移回未归幕仅解除归属、保留正文位置；删除幕保留全部场景和素材。零幕工程保存重开不会自动新增第一幕。
- 新增幕分组模型及 UI 回归，核心契约与 CI 同步更新；不改写作快捷键、自由板或 PDF 导出逻辑。

## v1.3.0-alpha.18.4（本地修复，待发布）

- 加固故事板归幕/未归幕拖放：拖动期间保持空梗概源节点，稳定场景卡标识；修复向后插入目标卡前的位置偏移，单次撤销完整恢复归幕和正文顺序。
- 新增归幕往返、空组/标题/卡片落点、取消、整场排序、保存和撤销专项测试，纳入 CI 与核心功能红线。
